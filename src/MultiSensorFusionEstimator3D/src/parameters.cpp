...
int removal_nan_ = 0;  // 是否去除NaN点 
std::string OPERATION_MODE_;  // pure_lidar: 纯雷达 , lidar_imu: 融合IMU, lidar_wheel: 融合轮速, lidar_wheel_imu: 融合imu与轮速 
std::string POINTCLOUD_MATCHING_MODE_; // 激光匹配模式  - 直接法(NDT、GICP、ICP...), 特征点...
std::string LIDAR_TRACKING_MODE_;   // 激光跟踪类型 
std::string DOWNSAMPLING_TYPE_ ;  
std::string OUTLIERREMOVAL_TYPE_ ;  
int lidar_num_;    
float SCAN_PERIOD_;  
int SCAN_NUM_;
// 预处理参数
float voxel_grid_resolution_;   
float radiusOutlierRemoval_radius_;  
int radiusOutlierRemoval_minNum_;  
float distanceFilter_max_;
float distanceFilter_min_;

int scan_map_windows_size_;  
...
{
    ParametersReader param_read(config_file);    // 参数读取器 
    param_read.ReadParam("lidar_num", lidar_num_); 
    param_read.ReadParam("operation_mode", OPERATION_MODE_); 
    param_read.ReadParam("pointcloud_matching_mode", POINTCLOUD_MATCHING_MODE_);
    param_read.ReadParam("lidar_tracking_mode", LIDAR_TRACKING_MODE_);
    param_read.ReadParam("scan_period", SCAN_PERIOD_);  
    param_read.ReadParam("lidar_scan", SCAN_NUM_);
    // 点云预处理参数   
    param_read.ReadParam("preprocess.downsampling.method", DOWNSAMPLING_TYPE_);
    param_read.ReadParam("preprocess.downsampling.voxel_grid.cell_size", voxel_grid_resolution_);
    param_read.ReadParam("preprocess.removalNaN", removal_nan_);

    param_read.ReadParam("preprocess.outlierRemoval.method", OUTLIERREMOVAL_TYPE_);
    if (OUTLIERREMOVAL_TYPE_ == "Geometric")
    {
        param_read.ReadParam("preprocess.outlierRemoval.geometric.radius", radiusOutlierRemoval_radius_);
        param_read.ReadParam("preprocess.outlierRemoval.geometric.min_num", radiusOutlierRemoval_minNum_);
    }
    
    param_read.ReadParam("preprocess.distanceFilter.max", distanceFilter_max_);
    param_read.ReadParam("preprocess.distanceFilter.min", distanceFilter_min_);

    // tracking 
    param_read.ReadParam("tracker.scan_map.sliding_window_size", scan_map_windows_size_);
    
    
    init_flag_ = true;  
    // OPERATION_MODE_ =  (std::string)fsSettings["operation_mode"];
    // LIDAR_TRACKING_TYPE_ =  (std::string)fsSettings["lidar_tracking_type"];
    // std::cout << common::GREEN << "LIDAR_TRACKING_TYPE_:" << LIDAR_TRACKING_TYPE_<<std::endl;
