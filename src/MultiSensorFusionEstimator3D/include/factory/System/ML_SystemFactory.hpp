 ...
 * @Author: 作者
 * @Version: 版本
 * @Date: 2022-04-10 17:09:28
 * @Description: 多激光系统构建工厂
 *                                多激光建图 + 定位 ，没有其他传感器 
 */
#pragma once 

#include "Common/read_param.hpp"
#include "System/ML_System.hpp"   // 多激光SLAM 系统 
...
class MultiLidarSystemFactory 
{   
    private:
        using SystemType = std::unique_ptr<MultiLidarSystem<_PointType, _FeatureType>>; 
        // 特征处理类型
        using FeatureProcessorPtr = typename MultiLidarSystem<_PointType, 
            _FeatureType>::FeatureProcessorPtr;
        // 激光tracker 类型 
        using LidarTrackerPtr = typename MultiLidarSystem<_PointType,
                _FeatureType>::LidarTrackerPtr;
    public:

        MultiLidarSystemFactory() {}
        ...
        SystemType Create(std::string config_path)
        {
            ...
            system_ptr.reset(new MultiLidarSystem<_PointType, _FeatureType>(
