...
namespace Slam3D {
    ...
    using XYZCorrectorPtr = std::unique_ptr<CorrectorInterface<_StateType, 
                                                                                        Eigen::Vector3d, _ErrorStateDim>>;
    ...
    : public FilterEstimatorBase<_StateType, _ErrorStateDim, ImuDataConstPtr, GnssDataConstPtr> 
    {
            ...
            public:
                ...
                InsEstimator(float const& acc_noise, 
                                          ...
                                          std::unique_ptr<ImuMotionModelInterface> imu_motion_model_ptr
                                            = std::make_unique<ImuMidIntegralModel>()  // 默认中值积分
                                          ) : imu_predictor_(acc_noise, gyro_noise, acc_bias_noise, gyro_bias_noise, 
                                                std::move(imu_motion_model_ptr)), 
                                                corrector_ptr_(std::move(corrector_ptr)) {}
                ...
                virtual bool Initialize(double const& timestamp) override {
                    ...
                    if (!imu_initialized_tool_.GetInfoFromImuData(acc_buf, gyro_buf, rot_buf, 
                            initialized_imu->acc, initialized_imu->gyro, initialized_imu->rot)) 
                    {
                ...
                virtual void Process(GnssDataConstPtr const& data) override 
                {
                    ...
                    {  
                        ...
                        { 
                            ...
                            // GNSS初始化 
                            GnssDataProcess &gnss_data_process = GnssDataProcess::GetInstance(); 
                    ...
                    {  // 如果初始化了 执行滤波器校正  
                        ...
                        if (gnss_processor.UpdateXYZ(data->lla)) 
                        {
                            ...
                            corrector_ptr_->Correct(position_obs, data->timestamp, data->cov, 
                                                                                BaseType::estimated_state_, BaseType::cov_);  
            ...
            private:
                ...
                std::unique_ptr<CorrectorInterface<_StateType, Eigen::Vector3d, 
                    _ErrorStateDim>> corrector_ptr_;  // 位置校正器
                ImuInitializedTool imu_initialized_tool_;    // IMU 初始化工具  
                std::vector<ImuDataConstPtr> prepared_imus_;   // 用于初始化的imu数据 
    ...
    void InsEstimator<StatesWithImu, 15>::statesCovarianceInitialize() 
    {
        ...
        BaseType::cov_.block<2, 2>(6, 6) = 10. * CoefDegreeToRadian * 10. * CoefDegreeToRadian 
