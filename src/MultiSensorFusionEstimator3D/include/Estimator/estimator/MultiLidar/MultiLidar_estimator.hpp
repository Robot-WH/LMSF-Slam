...
#include "BackEnd/g2o_backend.hpp"
#include <omp.h>  
...
namespace Slam3D{
    ...
    /**
     * @brief:  基于LOAM的多激光融合估计器
     * @param _PointT 输入的点的类型
     * @param _FeatureT 点云处理后得到的特征类型 
     */    
    template<typename _PointT, typename _FeatureT>
    class MultiLidarEstimator : public MultiLidarEstimatorBase<_PointT>
    {
        public:
            using Base = MultiLidarEstimatorBase<_PointT>; 
        ...
        private:
            ...
            std::unique_ptr<BackEndOptimizationBase<_FeatureT>> backend_;    // 后端优化模块 可选g2o/gtsam  
            ...
            std::queue<std::vector<FeatureInfo<_FeatureT>>> feature_buf_;     
            std::vector<FeatureInfo<_FeatureT>> cur_feature_;   // 当前特征数据  
        ...
        public:
            ...
            : MultiLidarEstimatorBase<_PointT>(NUM_OF_LASER)
            , processor_(std::move(processor)), lidar_tracker_(std::move(lidar_tracker))
            {
                ...
                backend_ = std::unique_ptr<BackEndOptimizationG2O<_FeatureT>>(
                    new BackEndOptimizationG2O<_FeatureT>());
                pose_lidar_cur_.resize(Base::NUM_OF_LIDAR_, Eigen::Isometry3d::Identity());  
                lidar_lidar_estrinsic_.setIdentity();  
                // 数据管理器注册
                DataManager::GetInstance().Registration<MultiLidarResultInfo<_FeatureT>>("frontend_info", 10); 
                DataManager::GetInstance().Registration<MultiLidarResultInfo<_FeatureT>>("backend_info", 10);
            }
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            /**
             * @brief: 对于Lidar数据的处理
            ...
            {        
                ...
                std::vector<FeatureInfo<_FeatureT>> all_lidar_feature;       // 每一个激光雷达的特征数据
            ...
            virtual void process() override
            {
                ...
                if (Base::EXTRINSIC_ESTIMATE_ == 1 && Base::EXTRINSIC_CALIB_STATUS_ != 2)
                {
                    ...
                    if (Base::EXTRINSIC_CALIB_STATUS_ == 0)
                    {
                        ...
                        {   // 运行里程计   
                            // TicToc tt;  
                            // tt.tic();  
                            FeatureInfo<_FeatureT>& cur_cloud_feature = cur_feature_[id];   // 获取特征数据 
                ...
                FeatureInfo<_FeatureT>& cur_cloud_feature = cur_feature_[0];   // 获取特征数据 
                lidar_tracker_[0]->Solve(cur_cloud_feature, delta_pose_[0]);   
