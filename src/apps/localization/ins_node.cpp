...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
GnssImuFilterFusionBridge(std::unique_ptr<LidarImuGnssFilterEstimatorInterFace> &estimator_ptr)
    : estimator_ptr_(std::move(estimator_ptr)) {   
    ...
    subImu = private_nh_.subscribe(imu_topic, 1000, 
                                                    &GnssImuFilterFusionBridge::imuHandler, this, 
                                                    ros::TransportHints().tcpNoDelay());

    subGnss = private_nh_.subscribe(gps_topic, 1000, 
                                                    &GnssImuFilterFusionBridge::gnssHandler, 
        ...
        if(!imu_buf.empty()||!gnss_buf.empty()) {
            ...
            switch(sensor_id) {
                ...
                case imu: {
                    ...
                    imu_buf.pop();  
                    estimator_ptr_->ProcessSensorData(imu_ptr);      // 将数据传入到估计器中进行
                    // 显示当前IMU的预测轨迹
...
int main(int argc, char **argv)
{
    ...
    // 融合线程  
    std::thread measurement_process{&FusionOdometryBridgeInterface::Process, 
                                                                                fusion_odometry_bridge.get()};
    ros::spin();
