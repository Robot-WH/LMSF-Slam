...
struct LidarEdgeFactor
{
    ...
    // 最核心   表示残差
	template <typename T>       // 计算点线残差  
	bool operator()(const T *q, const T *t, T *residual) const      // 重载 ()
	{
	    // 就是论文里的edge的cost function的公式
...
struct LidarPlaneFactor
{
	...
	bool operator()(const T *q, const T *t, T *residual) const
	{
		...
		Eigen::Matrix<T, 3, 1> lpj{T(last_point_j.x()), T(last_point_j.y()), T(last_point_j.z())};     // 平面点 j
		//Eigen::Matrix<T, 3, 1> lpl{T(last_point_l.x()), T(last_point_l.y()), T(last_point_l.z())};
		//Eigen::Matrix<T, 3, 1> lpm{T(last_point_m.x()), T(last_point_m.y()), T(last_point_m.z())};
		Eigen::Matrix<T, 3, 1> ljm{T(ljm_norm.x()), T(ljm_norm.y()), T(ljm_norm.z())};                 // 平面法向量
									   ...
									   const double s_)
	{
		return (new ceres::AutoDiffCostFunction<
				LidarPlaneFactor, 1, 4, 3>(        // 残差维度 1    优化变量 ： 7 
