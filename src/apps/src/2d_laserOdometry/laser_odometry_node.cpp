...
class SlamKarto
{
  ...
  private:
    ...
    std::string odom_frame_;  // odom系，机器人初始时刻位置设为odom坐标系原点
    std::string map_frame_;   // map系，世界坐标系，odom初始时刻位置设为map坐标系原点
    std::string base_frame_;  // base系，机器人坐标系
    int throttle_scans_;
...
SlamKarto::getLaser(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  {
    // New laser; need to create a Karto device for it.    新的激光类型数据
...
/**
 * t时刻机器人在odom系下的位姿
 * 机器人零时刻坐标系初始化为odom坐标系，那么之后时刻机器人的odom位姿，就是相对于零时刻机器人的位姿变化量；
 * 每时刻机器人的odom位姿来自Lidar数据集，可能是由IMU积分、轮式里程计等方法得到；
 * 这个位姿是严重漂移的，如果不经优化直接拿来建图，那么整个图都是错误的。
*/
bool
...
/**
 * 处理一帧数据，调用process主流程           laserCallback 中调用 
*/
bool
                   ...
                   karto::Pose2& karto_pose)
{
  ...
  std::vector<kt_double> readings;                 // 激光数据容器  
