...
visualization_msgs::MarkerArray createMarkerArray(KeyFrameInfo<PointT> const& info) 
{
    ...
    // 数量
    traj_marker.points.resize(info.keyframe_database_.size() + 
                                                            info.new_keyframes_.size());
    // 颜色
    traj_marker.colors.resize(info.keyframe_database_.size() + 
                                                            info.new_keyframes_.size());
    // 新增位姿节点
    for(int i=0; i<info.new_keyframes_.size(); i++) 
    {
        // 设置位置
        Eigen::Vector3d pos = info.new_keyframes_[i].correct_pose_.translation();
    ...
    // 关键帧数据库的位姿节点 
    for (int i=0; i<info.keyframe_database_.size(); i++) 
    {
        // 设置位置
        Eigen::Vector3d pos = info.keyframe_database_[i].correct_pose_.translation();
    ...
    // 这里要注意 ！！！！！！！！！！！！！！！！！
    edge_marker.points.resize(info.keyframe_database_.size() * 2 * 2 * 2 ); // + Loops.size() * 2);
    edge_marker.colors.resize(info.keyframe_database_.size() * 2 * 2 * 2 ); // + Loops.size() * 2);
    int i=0;

    for (int num = 0; num < info.keyframe_database_.size(); num++) 
    {
        // 里程计边    Pc
...
void processResult()
{
    while(1)
    {
        ...
        // 发布odom->map的变换
        static Eigen::Isometry3d trans_odom2map = Eigen::Isometry3d::Identity();
